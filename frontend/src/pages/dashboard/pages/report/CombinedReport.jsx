...
const CombinedReport = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [expRes, attRes, payRes, recRes] = await Promise.all([
      ...
      // Incoming = receipts only (as per requirement)
      const totalIncoming = totalReceipts;
      // Outgoing = expense + salary + pays
...
const handlePDFDownload = () => {
  ...
  try {
    ...
    // ── Load font (same as the working snippet) ──
    doc.addFileToVFS("Vazirmatn.ttf", VazirmatnTTF);
    doc.addFont("Vazirmatn.ttf", "Vazirmatn", "normal");
    doc.setFont("Vazirmatn", "normal"); // explicit style
    ...
    // ── Helper: Add a table for a category ──
    const addCategoryTable = (title, items, columns, total, count, startY) => {
      ...
      autoTable(doc, {
        ...
        headStyles: {
          font: "Vazirmatn",
          fontStyle: "normal",          // ← crucial for correct rendering
    ...
    doc.setFontSize(14);
    doc.setFont("Vazirmatn", "normal");
    doc.text("خلاصه مالی", rightX, y, { align: "right" });
    y += 25;
    ...
    lineY += 18;
    doc.text(`پرداخت‌هابه فروشندگان پلیت: ${formatNum(report.summary.totalPays)}`, rightX, lineY, { align: "right" });
    lineY += 18;
    ...
    // Pays
    tableY = addCategoryWithTitle(
      "پرداخت‌ها (دریافتی از فروشندگان)",
      report.pays.list,
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-green-50 p-4 rounded shadow">
              <div className="text-sm text-gray-600">پرداخت‌ها (دریافتی)</div>
              <div className="text-xl font-bold text-green-700">{formatNum(report.pays.total)}</div>
              <div className="text-xs text-gray-500">{report.pays.count} مورد</div>
            </div>
            <div className="bg-red-50 p-4 rounded shadow">
              <div className="text-sm text-gray-600">هزینه‌ها (خروجی)</div>
